...
module_scan_server <- function(input, output, session, get_selected_variable) {
...
module_scan_ui <- function(id) {
  ...
  tagList(
    ...
    div(
      id = ns("no_data"),
      box(h2("Your workspace does not contain any scan iso file objects."), width = 12)
    ) %>% shinyjs::hidden(),
  )
}
